...
> [!NOTE]  
> C1,C2,C3 커밋이 있을 때, `git reset --hard C2` 명령어를 실행하면 C3 커밋이 삭제되고, 작업트리와 스테이징이 C2 커밋 상태로 돌아갑니다.
...
```
> [!NOTE]  
> C1,C2,C3 커밋이 있을 때, `git revert C3` 명령어를 실행하면 C3 커밋의 변경 사항을 되돌리는(C2 커밋 상태) 새로운 커밋 C4가 생성됩니다. 기존 커밋들은 그대로 유지됩니다.
