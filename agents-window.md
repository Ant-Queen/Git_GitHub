...
Source: https://code.visualstudio.com/docs/agents/agents-window  
Fetched: 2026-06-24
...
In this article, you learn about the Agents window and how to start and manage agent sessions across your projects. For chat mechanics that apply to both the Agents window and the [Chat view](/docs/agents/chat-view) — such as sending requests, adding context, and reviewing changes — see [Use chat in VS Code](/docs/chat/chat-overview).

> Tip
>
> The Agents window (agent-first) and the [Chat view](/docs/agents/chat-view) (code-first) are the main surfaces for working with agents. They share the same sessions and settings, so you can move freely between them. For help choosing, see [Choose how you work with agents](/docs/agents/overview#_choose-how-you-work-with-agents).
...
*   Access to GitHub Copilot. Follow the steps in [Set up GitHub Copilot in VS Code](/docs/setup/copilot) to sign in and activate your subscription.
...
*   In VS Code, select the **Open in Agents** button in the title bar, or run **Chat: Open Agents Window** from the Command Palette (⇧⌘P (Windows, Linux Ctrl+Shift+P)).
    
    You can also open the Agents window directly from the VS Code welcome page.
    
    ![Screenshot showing how to open the Agents window from the Open in Agents button in the title bar and from the Welcome page.](/assets/docs/agents/agents-window/vscode-open-in-agents.png)
    
*   Run `code --agents` from the command line.
    
*   Open [https://insiders.vscode.dev/agents](https://insiders.vscode.dev/agents) in a browser to use the Agents window from any device. See [remote agent sessions](/docs/agents/remote-agent-sessions#_use-the-agents-window-in-the-browser) for setup instructions.
...
3.  **Chat area**: in the center, where you see the chat conversation history and where you can interact with the agent through prompts. You can [open multiple session views side by side](#_open-multiple-sessions-side-by-side) to compare or review work in parallel.
...
![Screenshot of the Agents window interface, showing the sessions list, customizations panel, chat area, and changes panel.](/assets/docs/agents/agents-window/agents-window-ui-annotated.png)
...
1.  Select **New** at the top of the sidebar or press ⌘N (Windows, Linux Ctrl+N).
    ...
    ![Screenshot showing how to start a new agent session by selecting New at the top of the sidebar in the Agents window.](/assets/docs/agents/agents-window/agents-window-new-session.png)
...
2.  Use the workspace dropdown to select a local folder or GitHub repository for your chat session.
    ...
    If the folder or repository you select isn't trusted yet, you'll be [prompted to trust](#_trust-a-folder) it before you can start a session.
    
    > Tip
    
    > You can track and create sessions that run on a remote machine via SSH or a dev tunnel. See [remote agent sessions](/docs/agents/remote-agent-sessions) for more information.
...
4.  Optionally, select extra configuration options for the session like a custom agent, language model, permission level, and more.
    ...
    You can change these at any point during the session. Learn more about [configuring your agent session](/docs/agents/overview).
    
5.  Type a prompt that describes what you want to accomplish, and press Enter.
    

The agent now starts working on your request. Learn more about [interacting in chat](/docs/chat/chat-overview).
...
By default, sessions are grouped by workspace and you can also group on timeframe. See [Manage Sessions](/docs/chat/chat-sessions) for more details on working with the session list.
...
![Screenshot showing the sessions list in the sidebar in the Agents window.](/assets/docs/agents/agents-window/agents-window-session-list.png)
...
![Screenshot showing the diff view in a modal window in the Agents window, with the layout controls in the diff view toolbar visible.](/assets/docs/agents/agents-window/agents-window-diff-view.png)
...
2.  Enter a comment that describes the change you want. The comment is anchored to the selected range.
    ...
    ![Screenshot showing how to add feedback from the diff view in the Agents window, with the Add Feedback button visible in the toolbar.](/assets/docs/agents/agents-window/agents-window-add-feedback.png)
...
![Screenshot showing the Changes panel in the Agents window, with the Files and Changes views visible.](/assets/docs/agents/agents-window/agents-window-changes.png)
...
2.  Select the **Tasks** dropdown in the title bar and select **Add Task**.
    ...
    ![Screenshot showing the Add Task dialog in the Agents window, where you can configure a task to run in the context of the current session.](/assets/docs/agents/agents-window/agents-window-add-task.png)
...
If your application involves browser-based behavior, you can use the [integrated browser](/docs/debugtest/integrated-browser) in the Agents window. Select a `localhost` link from the chat session to open it in the integrated browser inside the Agents window.
...
![Screenshot showing the integrated browser open in the Agents window, displaying a localhost page that was opened from a link in the chat session.](/assets/docs/agents/agents-window/agents-window-integrated-browser.png)
...
*   **Dev tunnels**: connect to a machine running a [dev tunnel](/docs/remote/tunnels) to start sessions or check in on existing ones.
    

Learn more about setting up and using remote connections in [remote agent sessions](/docs/agents/remote-agent-sessions).
...
1.  In an active session, select **New Chat** (`+`) in the session toolbar.
    ...
    ![Screenshot showing how a new chat tab appears in the chat area alongside the existing chat tab in the Agents window.](/assets/docs/agents/agents-window/agents-window-new-subsession.png)
...
> To explore an alternative direction from a specific point in a session's conversation, [fork the session](/docs/chat/chat-sessions#_fork-a-chat-session). Forking a session creates a new independent session with a copy of the conversation history up to a specific point.
...
*   Press through to focus a session by its position in the grid, from left to right.
*   Press to close all open sessions and return to the new-session view. This shortcut applies when a session has focus.
...
|---|---|
| **Agents** | Define custom agent personas with specific tools and instructions. [Learn more](/docs/agent-customization/custom-agents). |
| **Skills** | Add portable instruction folders that agents load when relevant. [Learn more](/docs/agent-customization/agent-skills). |
| **Instructions** | Set guidelines that shape how the AI generates code. [Learn more](/docs/agent-customization/custom-instructions). |
| **Hooks** | Run shell commands at lifecycle points during agent sessions. [Learn more](/docs/agent-customization/hooks). |
| **MCP Servers** | Connect AI to external tools and services via the MCP standard. [Learn more](/docs/agent-customization/mcp-servers). |
| **Plugins** | Install prepackaged bundles of customizations. [Learn more](/docs/agent-customization/agent-plugins). |
...
![Screenshot showing the Agent Customizations panel in the Agents window, with the list of available customizations visible.](/assets/docs/agents/agents-window/agents-window-customizations.png)
...
![Screenshot of the folder trust prompt in the Agents window, asking the user to trust the folder before starting an agent session.](/assets/docs/agents/agents-window/agents-window-folder-trust.png)

The Agents window shares the same workspace trust state with the main VS Code window. If you trust a folder in VS Code, it is also trusted in the Agents window, and vice versa. Learn more about workspace trust in the [Workspace Trust documentation](/docs/editing/workspaces/workspace-trust).
...
**When should I use the Agents window?**
...
**Can I continue sessions started in the main VS Code window in the Agents window?**
...
**Can I use the Agents window with local or third-party CLI agents?**
...
*   [Chat overview](/docs/chat/chat-overview) - add context, write effective prompts, and review changes.
*   [Manage chat sessions](/docs/chat/chat-sessions) - checkpoints, forking, archiving, and exporting sessions.
*   [Remote agent sessions](/docs/agents/remote-agent-sessions) - SSH, dev tunnels, and browser-based access.

---

(Original content preserved from Visual Studio Code docs; minor formatting/typo fixes applied during conversion.)
